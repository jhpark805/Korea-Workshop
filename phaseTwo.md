...
* Rholang의 기본원리 재논의
* RChain의 CBC Casper 컨센서스 프로토콜(Ethereum의 블라드 잠피르 공동 개발)
* 캐스퍼 프렌들리 유령
* 보안 RChain 네트워크를 시작하는 방법
* Rholang의 공식 의미
* RChain Platform 개발 현황
* RChain 토큰 및 지갑이 Ethereum보다 확장성이 뛰어난 이유
* Rholang 및 Byzantine 내 결함성
* 영문 한국어 기술 어휘 교환
* Rholang의 특정 주제
...
Rholang의 코드 어드벤처로부터 모든 문제를 해결하라. 입력파일을 읽을 필요는 없고, 당신의 코드에 적은예시의 입력을 넣으면 된다.

**2. BitStory와 같이 플레이 가능한 Casper 게임(GhostStory) 만들기(4점)**

비트스토리는 작업증명서을 배울수 있는 재미있는 방법이다. 우리는 아직 캐스퍼 게임을 가지고 있지 않다. 그러한 게임을 설명하고 증명하라.

**3. 두 개의 웹 페이지로 nth-caller 게임 구분(3점)**

게임을 주최하려는 사용자를 위한 한 페이지. 게임을 하고 싶은 사람들을 위한 또 다른 한 페이지.

**4. 모든 실습에서 rholang 튜토리얼(각 수업당 3점)**

각각의 수업은 몇 가지 프로그래밍 연습을 포함한다. 한 번 수업의 모든 실습을 완료하십시오.

**5. [연습으로 푸는 Rholang](https://github.com/JoshOrndorff/LearnRholangByExample)번역 (각 레슨 3점)]**

결과를 GitHub에 업로드하십시오.

**6. Rholang 최대공약수 구하기(3점)**

두 정수 중 [최대공약수](https://en.wikipedia.org/wiki/Greatest_common_divisor)]을(를) 계산할 수 있는 프로그램 작성

**7. Rholang SumList(2점)**
모든 정수를 목록에 추가하는 Rholang 코드를 작성하라. 예를 들면

```
sumList!([1, 2, 3, 4, 5], *답 // gives 15
```

**8. Rholang 계승(2점)**

정수를 사용하고 그 정수의 인수를 계산하는 Rholang 코드를 작성하라. 예: factorial!(3) = 6
...
Rholang 계약서에 정수를 기입하고 그것이 소수인지 여부를 말하라.

**11. Rholang Int to String(2점)**

Rholang은 정수를 문자열로 바꾸는 좋은 방법이 없다. 그렇게 하는 코드를 쓰습시오.

**12. Rholang 커닝페이퍼 번역(3점)**

**13. 캐스퍼 친화적인 최종 사용자 지정 도구에 대한 미니 레슨 작성(5점)**

이더리움은 또한 Casper of interest를 사용할 계획이다. 하지만 그들은 현재 일의 증거를 사용한다. 그들이 "친화적인 Finality Gadget"이라고 불리는 하이브리드 시스템을 사용해 전환한다

https://arxiv.org/pdf/1710.09437.pdf

https://vitalik.ca/files/casper_note.html

아이작과 조시는 카스퍼 FFG에 대해 잘 모른다. FFG에 대해 짧은 레슨을 쓰십시오.

**14. nth-caller dApp 실행(4점)**

어제 우리는 n번째 호출자 dApp (분산된 앱)이 rchain에서 실행되는 것을 보았다. 직접 당신의 RNode에서 dApp을 실행하십시오.
...
dApp 상태는 사용자가 nonce를 기억하고 사용자 인터페이스에 입력하게 한다. 다음에 어떤 nonce이 올 것인지 블록체인에게 물어보는 것이 프로그램에게 더 좋을 것이다.

**17. sha256 해시가 일곱개의 0으로 시작하는 텍스트 찾기(2점)**

당신은 이 텍스트를 찾는 어떤 프로그래밍 언어로든 당신 자신의 프로그램을 써야 한다. 기존 해싱 함수를 사용해야 하지만, 직접 메인 프로그램을 작성해야 한다.

(Rholang을 사용할 경우 +4점)

**18. 부울 연산자 또는 람다 미적분에 인코딩(4점)**

이삭은 람다 미적분학에서 AND 연산자를 만드는 방법을 보여주었는데, 당신은 이해했는가? 아니면?

**19. 람다 미적분학에 부울 연산자 코드 입력(3점)**

**20. 람다 미적분으로 정수를 인코딩하는 방법 설명 및 시연(5점)**

정수를 인코딩하는 기존 방법이 있다(부울란스 부호화 방법 등).

**21. 캐스퍼 정수 일치(3점)**

Casper Binary가 어떻게 수업시간에 일치했는지를 상기하고 그 과정을 정수로 일반화하시오.
...
**24. 정수 추가 및 곱셈을 지원하는 간단한 프로그래밍 언어 쓰기(5점)**

정의를 쓰고 그것이 시험 프로그램을 써서 효과가 있다는 것을 증명하라. [여기에 코드 쓰기 및 실행](http://www.kframework.org/tool/run/)

**25. RChain과 아이콘(3점) 비교**

조나단은 한국에 아이콘이라고 불리는 큰 블록체인 프로젝트가 있다고 말한다. 아이콘의 부가가치 요약을 작성하여 RChain과 비교하십시오.

**26. 자체 RChain 네트워크 실행(5점)**

한 무리의 사람들이 RChain 네트워크를 만들고 싶을 때, 그들은 "제네시스 의식"을 행한다. (첫 번째 블록을 "제네시스 블록"이라고 하기 때문에). 당신 자신의 제네시의식을 완성하라.

https://rchain.atlassian.net/wiki/spaces/CORE/pages/428376065/User+guide+for+running+RNode

https://docs.google.com/document/d/1Z5Of7OVVeMGl2Fw054xrwpRmDmKCC-nAoIxtIIHD-Tc/edit#heading=h.acnwii91bpfi

**27. Colab 디스코드 서버 가입(1점)**

RChain 공동체는 전세계에 퍼져있고 우리는 디스코드를 통해 연락을 하고 있다. 사람들이 함께 배우고 코드를 배우기 위해 일하는 한 그룹은 "colab"이라고 불린다. 만약 당신의 그룹의 모든 사람들이 [가입한다면](https://discord.gg/2X7mDyR), 당신은 1점을 얻게 된다.


**28. 이 문서 번역(3점)**

**29. Rholang 튜토리얼 레슨 작성(3점)**

**30. 일반적인 블록체인에 대한 가치의 한글 resource(3점)**

한국어 블록체인 resource를 찾아 GitHub에 추가하십시오.

**31. RChain이 과 상호 작용을 시작해야 하는 한국 블록체인 조직의 목록(3점)**

**32. 캐스퍼 실수 찾기(1점)**

이 캐스퍼 게임에는 실수가 있다. 찾을 수 있습니까?
